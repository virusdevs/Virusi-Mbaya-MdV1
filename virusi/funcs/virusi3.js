import { serialize, decodeJid } from '../../lib/virusi6.js';
import path from 'path';
import fs from 'fs/promises';
import config from '../../set.cjs';
import { smsg } from '../../lib/virusi3.cjs';
import { handleAntilink } from './virusi5.js';
...
const Handler = async (chatUpdate, sock, logger) => {
    ...
    if (m.key && m.from) {
    // Handle presence updates for typing
    if (config.AUTO_TYPING) {
        await sock.sendPresenceUpdate("composing", m.from, [m.key]);
    }

    // Handle presence updates for recording
    if (config.AUTO_RECORDING) {
        await sock.sendPresenceUpdate("recording", m.from, [m.key]);
    }

    // Handle presence updates for always online status
    if (config.ALWAYS_ONLINE) {
        await sock.sendPresenceUpdate('available', m.from, [m.key]);
    } else {
        await sock.sendPresenceUpdate('unavailable', m.from, [m.key]);
    }
    ...
    // Auto-block specific users if enabled
   ...
   // Anti-delete functionality
    if (m.messageStubType === 68 && config.ANTI_DELETE) { // 68 is the stub type for message deletions
      ...
      await sock.sendMessage(config.OWNER_NUMBER + '@s.whatsapp.net', { text: messageToSend });
    }                                                           
}
        ...
        const pluginDir = path.join(__dirname, '..', 'cmds');
